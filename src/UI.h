...
int ConsoleUI::inputInt(const string& prompt) {
    int value;
    while (true) {
        cout << prompt;
        cin >> value;
...
double ConsoleUI::inputDouble(const string& prompt) {
    double value;
    while (true) {
        cout << prompt;
        cin >> value;
...
string ConsoleUI::inputString(const string& prompt) {
    string value;
    cout << prompt;
    getline(cin, value);
    return value;
...
ConsoleUI::ConsoleUI(Bank& b) : bank(b) {}
...
void ConsoleUI::showAllAccounts() {
    cout << "\n=== ВСЕ АККАУНТЫ ===\n";
    const vector<Account>& accounts = bank.getAccounts();
